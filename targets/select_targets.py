...
def _github_repo_info(repo_url: str) -> Optional[Dict]:
    ...
    if resp.status_code != 200:
        return None
    data = resp.json()
    return {"language": data.get("language"), "archived": data.get("archived", False)}
...
def select_targets(program_handle: Optional[str] = None) -> List[Dict]:
    ...
    candidates.sort(key=lambda c: (c["tier"], c["repo_url"]))
    return candidates
...
def main():
    ...
    for c in candidates:
        print(f"  [tier {c['tier']}] {c['language']:6s} {c['repo_url']} ({c['program_name']})")
